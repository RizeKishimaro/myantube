...
const ActionBar = ({ actionData }) => {
  ...
  return (
    <div className={"flex justify-between h-15 py-3"}>
      ...
      <ThumbUpIcon /> 
        <span className={`ml-2 text-sm text-center my-auto`} onClick={()=>{setLike(likeam =>{ return likeam +1})}}>{like || 0}</span>
      </button>
      <button className="border border-white px-4 py-1 rounded-full flex w-[100px]">
        <ThumbDownIcon /> 
        <span className={`ml-2 text-sm text-center my-auto`} onClick={()=>{setDislike(dislikeam => dislikeam + 1)}}>{dislike ||0}</span>
      </button>
